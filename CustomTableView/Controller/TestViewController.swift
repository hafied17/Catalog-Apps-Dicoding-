...
class TestViewController: UIViewController {
    ...
    private func saveFavorite() {
        favoriteProvider.getAllFavorite { [self] data in
            DispatchQueue.main.async {
                if data.contains(where: { $0.idGame!.contains(self.idgameF)}) {
                    let alert = UIAlertController(title: "Error", message: "The game is already on your favorites list!", preferredStyle: .alert)
                            
                    alert.addAction(UIAlertAction(title: "OK", style: .default) { _ in
                    })
                    self.present(alert, animated: true, completion: nil)
                } else {
                    self.favoriteProvider.createFavorite(self.describF, self.developerF, self.genreF, self.idgameF, self.imageF, self.nameF, self.ratingF, self.releasedF, self.webF) {
